...
| 4e. iPhone local-preview IDs | Complete in code; iPhone check pending | Tech lead | Replaced secure-context-only `crypto.randomUUID()` in app storage with UUID v4 generation from `crypto.getRandomValues()`, which also works over local-network HTTP. All 20 tests and the build pass; the iPhone preview needs a refresh to confirm. |
