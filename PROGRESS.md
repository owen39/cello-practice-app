...
| 4g. Offline iPhone PWA | HTTPS deployment prepared; publishing pending | Tech lead | The production app shell reloads with its server stopped on a trustworthy localhost origin. LAN HTTP on iPhone cannot register a service worker. A manual GitHub Pages workflow and subpath-safe manifest are ready; 20 tests and the Pages build pass. Publishing requires an eligible Pages plan and approval for a public HTTPS site. |
